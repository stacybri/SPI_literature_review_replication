...
ggplot(fig, aes(x=var, y=abs(SPI...1))) +
  ...
  theme_spi() +
  guides(
    fill=guide_legend('Guide'),
    color=guide_legend('Guide'),
    shape=guide_legend('Guide')
  ) +
...
Figure created using Microsoft Powerpoint.  Image shown below for completeness.

![](./other_figures/SPI_theory_change.png)



Note: the indexes with a correlation with the shown SDG outcome that is statistically not different from each other are shown in adjacent row. 
...
for (i in 1:n) {
  for (j in 1:c) {
    for (k in 1:c) {
      ...
      n_1 <- 100  # Sample size for correlation_1
      n_2 <- 150  # Sample size for correlation_2
      se_1 <- 1 / sqrt(n_1 - 3)
      se_2 <- 1 / sqrt(n_2 - 3)
...
flextable(pval_df) %>%
  set_header_labels(
    SDG="SDG",
    V1="GDB",V2="GDB",V3="GDB",V4="GDB",V5="GDB",
    V6="ODB",V7="ODB",V8="ODB",V9="ODB",V10="ODB",
    V11="ODIN",V12="ODIN",V13="ODIN",V14="ODIN",V15="ODIN",
    V16="SCI",V17="SCI",V118="SCI",V19="SCI",V20="SCI",
    V21="SPI",V22="SPI",V23="SPI",V24="SPI",V25="SPI"
  ) %>%
  merge_at(i=1,j=2:6, part = 'header') %>%
  merge_at(i=1,j=7:11, part = 'header') %>%
  merge_at(i=1,j=12:16, part = 'header') %>%
  merge_at(i=1,j=17:21, part = 'header') %>%
  merge_at(i=1,j=22:26, part = 'header') %>%
  vline(j=1) %>%
  vline(j=6) %>%
  vline(j=11) %>%
  vline(j=16) %>%
...
## Table A.10.  Relationship between the SDG Index Overall Score from the 2023 Sustainable Development Report and SPI scores, 2016-2022
...
## Table A.11. Relationship between Log GDP per capita and ODIN, Open Data Barometer, and Global Data Barometer scores, 2016-2022
...
## Table A.12. Relationship between Government Effectiveness from Worldwide Governance Indicators and ODIN, Open Data Barometer, and Global Data Barometer scores, 2013-2022
...
print(paste0("ODB Model 3: sigma_e=",round(sqrt(p$ercomp$sigma2[1]),2), ". sigma_u=",round(sqrt(p$ercomp$sigma2[2]),2)))
```
## Table A.13. Relationship between the SDG Index Overall Score from the 2023 Sustainable Development Report and ODIN, Open Data Barometer, and Global Data Barometer scores, 2013-2022
